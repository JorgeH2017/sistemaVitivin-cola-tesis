...
if (isset($_POST["btnmodificarcliente"]) && $_POST["btnmodificarcliente"] == "Modificar") {
    ...
    if (empty($_POST["txtnombrecliente"]) || $_POST["txtnombrecliente"] == "" || $_POST["txtnombrecliente"] == null || preg_match("/^\s+$/", $_POST["txtnombrecliente"])) {
        ...
        $resul = $objClie->modificarCliente();
        if ($resul != "") {
            echo "<script language='javascript'>alert('Datos modificados correctamente');window.location='../Vista/listarcliente.php'</script>";
        } else {
            echo "<script language='javascript'>alert('Error al modificar datos');window.location='../Vista/listarcliente.php'</script>";
        }
    }
}