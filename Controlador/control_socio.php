...
if (isset($_POST["btningresosocio"]) && $_POST["btningresosocio"] == "Ingresar") {
    ...
    if (empty($_POST["txtnombresocio"]) || $_POST["txtnombresocio"] == "" || $_POST["txtnombresocio"] == null || preg_match("/^\s+$/", $_POST["txtnombresocio"])) {
        ...
        echo "El apellido debe ser solo letra";
        ...
        $resul = $objSocio->existeRut($rutsocio);
        $num = mysqli_num_rows($resul);
        if ($num > 0) {
            echo "El rut del socio ya existe, debes ingresar otro";
        } else {
            if (isset($_POST["btningresosocio"]) && $_POST["btningresosocio"] == "Ingresar") {
                $objSocio = new Socio();
                $objSocio->Socio($rutsocio, $nombresocio, $apellidosocio, $telefonosocio, $correosocio, $direccionsocio, $idusuario, $tipousuario);
                $resul = $objSocio->insertarSocio();
                if ($resul != "") {
                    echo "<script language='javascript'>alert('Datos ingresados correctamente');window.location='../Vista/ingresarsocio.php'</script>";
                } else {
                    echo "<script language='javascript'>alert('Error al ingresar datos');window.location='../Vista/ingresarsocio.php'</script>";
                }
            }
        }
    }
