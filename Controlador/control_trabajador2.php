...
if (isset($_POST["btnmodificartrabajador"]) && $_POST["btnmodificartrabajador"] == "Modificar") {
    ...
    if (empty($_POST["txtnombretrabajador"]) || $_POST["txtnombretrabajador"] == "" || $_POST["txtnombretrabajador"] == null || preg_match("/^\s+$/", $_POST["txtnombretrabajador"])) {
        ...
        $resul = $objTrabajador->modificarTrabajador();
        if ($resul != "") {
            echo "<script language='javascript'>alert('Datos modificados correctamente');window.location='../Vista/listartrabajador.php'</script>";
        } else {
            echo "<script language='javascript'>alert('Error al modificar datos');window.location='../Vista/listartrabajador.php'</script>";
        }
    }
}